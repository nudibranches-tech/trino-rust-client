...
- Configurable retry/backoff via `RetryPolicy` (`max_attempts`, `min_delay`, `max_delay`, `jitter`) and `ClientBuilder::retry_policy`. The default preserves the previous behaviour (3 retries, 1s→2s, no jitter). The retryable-error predicate is now explicit and documented: transient responses (HTTP 502/503/504) and connect/timeout errors are retried; query, decode, protocol and other errors fail fast (previously only 503 was retried)
